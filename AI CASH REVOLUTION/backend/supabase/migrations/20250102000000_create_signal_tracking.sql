-- Signal Tracking System for Collective Learning
-- Traccia ogni segnale generato + outcome per ML optimization
...
CREATE TABLE IF NOT EXISTS collective_signals (
  ...
  -- Metadata
  signal_version VARCHAR(20), -- Per tracking algorithm changes
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Indexes for fast queries
CREATE INDEX idx_collective_signals_symbol ON collective_signals(symbol);
CREATE INDEX idx_collective_signals_status ON collective_signals(status);
CREATE INDEX idx_collective_signals_session ON collective_signals(session);
CREATE INDEX idx_collective_signals_regime ON collective_signals(regime);
CREATE INDEX idx_collective_signals_entry_time ON collective_signals(entry_time DESC);
CREATE INDEX idx_collective_signals_user ON collective_signals(user_id);
CREATE INDEX idx_collective_signals_outcome ON collective_signals(status, pnl);

-- Composite indexes for ML queries
CREATE INDEX idx_ml_features ON collective_signals(symbol, session, regime, status);
CREATE INDEX idx_ml_patterns ON collective_signals(pattern_m15, pattern_m5, status);
...
CREATE TABLE IF NOT EXISTS ml_training_log (
  ...
  context_filter JSONB, -- {symbol, session, regime}
