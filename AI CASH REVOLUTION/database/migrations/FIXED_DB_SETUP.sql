-- ========================================
-- ML COLLECTIVE LEARNING DATABASE SETUP (FIXED)
-- Versione corretta senza IF NOT EXISTS per policies
-- ========================================
...
DROP POLICY IF EXISTS "Users can read all signals" ON collective_signals;
DROP POLICY IF EXISTS "Users can insert their signals" ON collective_signals;
DROP POLICY IF EXISTS "Service role full access" ON collective_signals;
DROP POLICY IF EXISTS "Everyone can read ML weights" ON ml_weight_optimization;
DROP POLICY IF EXISTS "Service role can update weights" ON ml_weight_optimization;
DROP POLICY IF EXISTS "Everyone can read training log" ON ml_training_log;
DROP POLICY IF EXISTS "Service role can log training" ON ml_training_log;

-- Create policies (without IF NOT EXISTS)
CREATE POLICY "Users can read all signals"
  ON collective_signals FOR SELECT
  USING (true);

CREATE POLICY "Users can insert their signals"
...
CREATE POLICY "Everyone can read ML weights"
  ON ml_weight_optimization FOR SELECT
  USING (true);

CREATE POLICY "Service role can update weights"
  ON ml_weight_optimization FOR ALL
  USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY "Everyone can read training log"
  ON ml_training_log FOR SELECT
  USING (true);

CREATE POLICY "Service role can log training"
