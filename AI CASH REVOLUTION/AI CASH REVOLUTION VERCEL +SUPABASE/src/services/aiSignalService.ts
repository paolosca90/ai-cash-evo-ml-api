    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/generate-ai-signals`, {
        ...
        body: JSON.stringify({
          ...
          saveToDatabase: false,  // ⚠️ IMPORTANT: Never save to database from AI Signal generation
          analysisOnly: true      // ⚠️ IMPORTANT: Analysis only, no trade execution
        }),
  ...
  private async generateHybridSignal(symbol: string, timeout: number): Promise<AISignal> {
    try {
      ...
      const mlResult = await mlSignalService.generateSignal({
        symbol,
        timeout: timeout / 2, // Half timeout for ML
        useEnsemble: true,
        enableConstraints: true
      });

      // 3. If ML failed, return AI signal
      if (!mlResult.success || !mlResult.signal) {
        console.warn('⚠️ ML signal failed, using AI signal only');
        return aiSignal;
      }

      const mlSignal = mlResult.signal;
