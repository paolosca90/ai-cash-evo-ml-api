...
-- Create trigger for automatic notification (optional - can be enabled later)
-- This trigger would automatically call notify-signal when a new signal is inserted

-- Commented out for now - enable if needed
/*
CREATE OR REPLACE FUNCTION trigger_signal_notification()
RETURNS TRIGGER AS $$
BEGIN
    -- Call notify-signal function asynchronously
    PERFORM pg_notify('new_signal',
        json_build_object(
            'signal_id', NEW.id,
...
CREATE TRIGGER trigger_signal_notification
    AFTER INSERT ON mt5_signals
    FOR EACH ROW
    EXECUTE FUNCTION trigger_signal_notification();
*/
