...
-- Add constraints
ALTER TABLE signal_performance
ADD CONSTRAINT IF NOT EXISTS check_sp_signal_weight_range
CHECK (signal_weight IS NULL OR (signal_weight >= 0 AND signal_weight <= 100));

ALTER TABLE signal_performance
ADD CONSTRAINT IF NOT EXISTS check_sp_position_multiplier_range
CHECK (position_multiplier IS NULL OR (position_multiplier >= 0.25 AND position_multiplier <= 2.0));

ALTER TABLE signal_performance
ADD CONSTRAINT IF NOT EXISTS check_sp_signal_recommendation_values
