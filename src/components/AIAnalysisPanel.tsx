...
export const AIAnalysisPanel = ({ symbol }: AIAnalysisPanelProps) => {
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      // 🔧 FIX: Per BUY, assicurati che TP > SL e per SELL che TP < SL
      if (data.type === 'BUY') {
        // In un BUY, TP deve essere sopra l'entry e SL sotto
        if (data.takeProfit < data.stopLoss) {
          // Invertiti! Correggi
          const temp = data.takeProfit;
          data.takeProfit = data.stopLoss;
          data.stopLoss = temp;
          console.log('✅ Corretto SL/TP invertiti per BUY');
        }
      } else if (data.type === 'SELL') {
        // In un SELL, TP deve essere sotto l'entry e SL sopra
        if (data.takeProfit > data.stopLoss) {
          // Invertiti! Correggi
          const temp = data.takeProfit;
          data.takeProfit = data.stopLoss;
          data.stopLoss = temp;
          console.log('✅ Corretto SL/TP invertiti per SELL');
        }
      }
    ...
    <Card className="bg-card border-border">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          Ottieni segnali di trading professionali generati da intelligenza artificiale avanzata. Analisi multi-timeframe con entry, stop loss e take profit ottimali per massimizzare i tuoi profitti.
        </p>
      </CardHeader>
