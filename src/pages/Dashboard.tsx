...
const Dashboard = () => {
  ...
  const fetchMT5Signals = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // Fetch segnali MT5 dal database, ordinati per data più recente
      // SICUREZZA: Ora ogni utente vede solo i propri segnali di trading
      const { data: mt5Signals, error } = await supabase
        .from('mt5_signals')
        .select('*')
      ...
      toast({
        ...
        description: `${signals.length} segnali trovati nel database`
      });
    } catch (error) {
      console.error("Error fetching MT5 signals:", error);
      toast({
        title: "Errore",
        description: "Impossibile aggiornare i segnali MT5",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
    }
  }, [isAuthenticated, toast, calculateRealStats]);
